...
def my_simple_streamlit_app():
    ...
    st.set_page_config(page_title='Exploring Streamlit functionalities', layout = "wide", page_icon=':nerd_face:')
    ...
    # Let's add some text
    st.write("The purpose of this app is to learn the basics of Streamlit and how to use it to build a simple web app.")
    ...
    income_line_chart = alt.Chart(avg_income_per_year).mark_line(point=True).encode(
        x=alt.X('year:O', axis=alt.Axis(title='Year')),
        y=alt.Y('yearly_income:Q', axis=alt.Axis(title='Average income'), scale=alt.Scale(domain=[0, 60_000])),
        color=alt.value(NESTA_COLOURS[0]),
        tooltip=['year', 'yearly_income']
    ).properties(
        title='Income grows every year',
        width=400,
        height=500
    )
    ...
    col1, col2, col3 = st.columns([1, 2, 1])  # Adjust the ratio as needed - in this case column 2 is twice as wide as the others
